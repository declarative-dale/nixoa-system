...
This repository is the NiXOA user configuration layer. Primary files are `config/default.nix` (aggregates `config/`), `modules/hosts.nix`, `modules/host.nix`, `modules/user.nix`, `hardware-configuration.nix`, and optional `config.nixoa.toml`. Plain implementation modules live under `modules/_nixos/` and `modules/_user/`. Helper scripts are in `scripts/`, and documentation is under `docs/`.
...
- File naming is fixed: `config/default.nix` is the primary aggregator entry point.
